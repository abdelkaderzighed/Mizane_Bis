...
for i, row in enumerate(rows):
    try:
        # On ne migre QUE les métadonnées légères
        # Les données volumineuses (HTML, embeddings) iront dans R2
        pg_cur.execute("""
        ...
        """, (
            row['id'], row['decision_number'], row['decision_date'],
            ...
            row['file_path_ar'], row['file_path_fr'],  # URLs R2 existantes
        ...
        print(f"\n   ✗ Erreur decision {row['decision_number']}: {e}")
        pg_conn.rollback()

pg_conn.commit()
print(f"\r3️⃣ Migration supreme_court_decisions... ✅ {inserted}/{total} lignes" + " "*20)
