...
const HierarchicalView = () => {
  ...
  const viewDocument = (doc) => {
    ...
    const url = doc.statuts.downloaded 
      ? `${API_URL}/documents/${doc.id}/view`  // Endpoint à créer côté backend
  ...
  const downloadDocument = async (doc) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/documents/${doc.id}/metadata`);
      const data = await res.json();
  ...
  const loadDocuments = async (sessionId, page = 1) => {
    console.log('🔄 loadDocuments démarré pour session', sessionId, 'page', page);
    try {
      ...
      if (data.success) {
        console.log('Documents reçus:', data.documents.length, 'docs');
        setSessionDocuments(prev => {
          const newState = {...prev, [sessionId]: data};
          console.log('sessionDocuments mis à jour:', newState);
          return newState;
        });
  ...
  const toggleSession = (sessionId) => {
    console.log('toggleSession appelé:', sessionId);
    const wasExpanded = expandedSessions.has(sessionId);
    console.log('wasExpanded:', wasExpanded, 'sessionDocuments:', sessionDocuments);
              ...
              {expandedSites.has(site.id) && (
                site.id === 2 ? (
                  <div className="px-4 pb-4">
                    <CoursSupremeView siteId={site.id} />
                  </div>
                ) : (
                      ...
                      {expandedSessions.has(session.id) && (
                        <div className="mt-3 pl-6 space-y-2">
                          <div className="flex gap-2">
                            ...
                            <button 
                              onClick={() => analyzePhase(session.id)}
              ...
              {selectedDocMetadata.statuts?.analyzed ? (
                <>
                  <div className="mb-3">
                    ...
                    <p className="text-sm mt-1">{selectedDocMetadata.ai_analysis?.summary || 'Non disponible'}</p>
                  </div>
                  
                  <div className="mb-3">
                    <span className="font-medium text-sm">Mots-clés :</span>
                    {selectedDocMetadata.ai_analysis?.keywords ? (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {selectedDocMetadata.ai_analysis.keywords.split(',').map((kw, i) => (
                  ...
                  <div className="mb-3">
                    ...
                    <p className="text-sm mt-1">{selectedDocMetadata.ai_analysis?.entities || 'Non disponible'}</p>
                  </div>
                  
                  <div className="text-xs text-gray-600">
                    Texte extrait : {selectedDocMetadata.ai_analysis?.extracted_text_length || 0} caractères
                  </div>
                </>
              ) : (
