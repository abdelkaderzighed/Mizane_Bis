...
def _ensure_text_content(doc_id: int, file_path: str | None, text_path: str | None, url: str):
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE documents SET text_path = ? WHERE id = ?", (uploaded_text_url, doc_id))
    conn.commit()
    conn.close()
...
def get_document_metadata(doc_id):
    ...
    try:
        ...
        file_exists = _r2_exists(doc.get('file_path'))
        text_exists = _r2_exists(doc.get('text_path'))
...
def download_single_document(doc_id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, url, file_path, download_status
            FROM documents
            WHERE id = ?
        """, (doc_id,))
        ...
        file_exists = _r2_exists(doc['file_path'])
...
def get_session_documents(session_id):
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            file_exists = _r2_exists(row['file_path'])
            text_exists = _r2_exists(row['text_path'])
