# Module d'analyse IA - Généré automatiquement
...
def analyze_with_claude(text, api_key, document_info=None):
    if not ANTHROPIC_AVAILABLE:
        return False, None, 0, "anthropic non disponible"
    if not api_key:
    ...
    try:
        ...
        prompt = f'''Analysez ce document juridique.
...
def process_single_document(doc_id, api_key, stop_event=None):
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            success, analysis, tokens, error = analyze_with_claude(text, api_key, doc_info)
            if not success:
                result['error'] = error
                cursor.execute("UPDATE documents SET analysis_status = 'error' WHERE id = ?", (doc_id,))
                conn.commit()
